...
AIRE_COMMUNE_MAX_M2 = 30_000_000_000  # 30 000 km², une grande région
...
class Command(BaseCommand):
    ...
    def _importer_communes(self, provinces_geoms_3857):
        ...
        # les noms accentués (ex. "El Kelâa..."). LATIN1/CP1252 est
        # l'encodage courant des vieux DBF francophones (fichier de 2018,
        # confirmé empiriquement : décode proprement avec ce choix). Scopé à
        # cette lecture uniquement (restauré après), pour ne pas affecter un
        # éventuel futur import provinces relu dans le même process.
        encodage_precedent = os.environ.get('SHAPE_ENCODING')
        os.environ['SHAPE_ENCODING'] = 'LATIN1'
        try:
            ds = DataSource(str(COMMUNE_SHP))
            layer = ds[0]
            return self._lire_communes(layer, provinces_geoms_3857)
        finally:
            if encodage_precedent is None:
                os.environ.pop('SHAPE_ENCODING', None)
            else:
                os.environ['SHAPE_ENCODING'] = encodage_precedent
    ...
    def _lire_communes(self, layer, provinces_geoms_3857):
        ...
        for feat in layer:
            fid = feat.fid
            libelle = feat.get('LIBELLE')
            region_declaree = int(feat.get('REGION'))
